...
type Repository interface {
	...
	Get(ctx context.Context, id string) (entity.ApiKey, error)
	// Count returns the number of users.
	Count(ctx context.Context) (int, error)
	// Query returns the list of users with the given offset and limit.
	Query(ctx context.Context, offset, limit int) ([]entity.ApiKey, error)
	// Create saves a new user in the storage.
	Create(ctx context.Context, user entity.ApiKey) error
	// Update updates the user with given ID in the storage.
	Update(ctx context.Context, user entity.ApiKey) error
...
func (r repository) Get(ctx context.Context, id string) (entity.ApiKey, error) {
	var user entity.ApiKey
...
func (r repository) Create(ctx context.Context, user entity.ApiKey) error {
	return r.db.With(ctx).Model(&user).Exclude("IsActive").Insert()
}

// Update saves the changes to an user in the database.
func (r repository) Update(ctx context.Context, user entity.ApiKey) error {
...
func (r repository) Query(ctx context.Context, offset, limit int) ([]entity.ApiKey, error) {
	var users []entity.ApiKey
	err := r.db.With(ctx).
		Select().
		OrderBy("id").
		Offset(int64(offset)).
